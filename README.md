...
| **某个阶段无任何报错就中断了** | 撞上 `set -e` 静默退出。四个主流程脚本装了 `EXIT` 中断陷阱（`install.sh` / `kp-install.sh` / `kp-storage-init.sh` / `kp-ocspeed.sh`），会打印一行 `✗ …中断（rc=N）`。**提示语各带自己的阶段名**（引导器 / 脚本 / 存储初始化 / ocspeed 安装），所以要搜 `中断（rc=` 才能一网打尽；**它不给行号**，线索是倒数第二行输出。多半是 `X=$(cmd)` 少写了 `\|\| X=""`（见下） |

### 改脚本前必读：`set -e` 会静默退出

五个脚本都开了 `set -eu`（只读体检脚本 `kp-store-check.sh` 刻意只开 `set -u`，
诊断工具不该因为哪个查询返回非 0 就自己退出）。
`set -e` 下**命令返回非 0 就可能让脚本无声退出**。三条规则（第 1 条是
2026-09-16 在真机 busybox ash 上逐条实测后**修正**的，此前的写法是过度保守）：

1. **`[ x ] && y` 本身是安全的**，别误判、也别去改它 —— ash 遵循 POSIX 的
   AND-OR 豁免规则，实测 11 种形态（顶层 / 函数中段 / `[ x ] || y` / `false && …`）
   全都不触发退出。
   **唯一的例外：它是一条函数的最后一条语句、且该函数被裸调用时** ——
   `f() { [ -n "" ] && Y=1; }; f` 会让调用点直接退出（实测过）。
   所以函数末句要放必然返回 0 的命令（像 `ui_init` 结尾是 `ui_hr`）。
2. **每个 `X=$(cmd)` 后面都跟 `|| X=""`** —— 纯赋值语句的退出码 = 命令替换的退出码。
   典型凶手：`uci -q get <未设置的键>` 返回 1。**真正踩过两次的是这一条。**
3. **可能失败的裸命令也要兜底** —— `uci -q delete <不存在的键>`、`cp <不存在的文件>`、
   `/etc/init.d/xxx enable` 都会返回非 0，而它们不在 `&&`/`||` 列表里，会直接结束脚本。
   统一写 `|| :` 或 `|| ui_warn "..."`。

这三条是拿一份 12 用例的脚本在设备上逐条跑出来的（而不是照着经验推），
好处是**不会把本来就安全的写法也改一遍**，白添改动风险。
